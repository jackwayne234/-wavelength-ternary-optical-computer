...
| Direct performance comparisons to H100/B200 | Must be re-evaluated. Previous comparisons assumed fully optical MAC latency and power. Hybrid architecture adds photodetection latency (~ns) and electronic accumulation power. The comparisons may still be favorable, but the numbers need to be re-run with honest hybrid overhead. |
...
Next steps:
1. Re-run system-level power and throughput models with hybrid overhead (photodetection + electronic accumulate)
2. Update performance comparison tables against H100/B200 with honest numbers
3. Design the photodetector array and adder tree for the target vector dimension
4. Characterize the optical-to-electronic boundary: detector responsivity, noise floor, bit error rate at target data rates
