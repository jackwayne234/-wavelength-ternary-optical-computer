...
\title{Wavelength-Division Ternary Computing II:\\The N-Radix Optical AI Accelerator}
...
\subsection{\texorpdfstring{3\textsuperscript{3} Encoding}{3^3 Encoding}}
...
\subsection{\texorpdfstring{Why 3\textsuperscript{3}\textsuperscript{3}\textsuperscript{3} Is Beyond Engineering}{Why 3^3^3^3 Is Beyond Engineering}}
...
\subsection{\texorpdfstring{Throughput with 3\textsuperscript{3} Encoding}{Throughput with 3^3 Encoding}}
...
The optical compute core has no software attack surface—but the overall system still requires traditional cybersecurity. An attacker who compromises the host computer, network interface, or NR-IOC could manipulate data going into or out of the optical core. Standard security practices remain essential for these components.

What the attacker \emph{cannot} do is alter the computation itself. The optical core will always perform the exact operations defined by its physical geometry. There is no way to inject malicious logic, install persistent malware, or reprogram the accelerator remotely. To change what the optical core computes, you would need to physically alter the glass.

\subsection{The Benefit}

For applications where computational integrity matters—cryptographic operations, secure enclaves, critical infrastructure—this provides a unique guarantee: \textbf{the compute logic cannot be remotely modified}. Even if an attacker fully compromises the host system, they cannot change what the optical core does, only what data it receives.

This is not a replacement for traditional security—it is an additional layer. The electronic interfaces (NR-IOC, PCIe, host system) require standard protection. Manufacturing trust and supply chain security remain considerations. But the optical core itself provides a physically immutable computational foundation that traditional processors cannot offer.
