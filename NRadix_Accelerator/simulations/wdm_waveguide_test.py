...
Performance Note:
    ...
    /home/jackwayne/miniconda/envs/meep_env/bin/python wdm_waveguide_test.py
...
def main():
    ...
    print_master("\n" + "=" * 70)
    print_master("N-RADIX WDM WAVEGUIDE VALIDATION TEST")
    print_master("=" * 70)
    print_master(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    if IS_PARALLEL:
    ...
    print_master(f"Testing {len(wavelengths)} wavelengths from 6 triplets:")
    for i, triplet in WDM_TRIPLETS.items():
        print_master(f"  Triplet {i}: {triplet['lambda_neg']:.3f} / {triplet['lambda_zero']:.3f} / {triplet['lambda_pos']:.3f} um")
    ...
    # Summary
    print_master("\n" + "=" * 70)
    print_master("SUMMARY")
    print_master("=" * 70)
    if passed:
        print_master("SUCCESS: All 18 wavelengths propagate through waveguide independently.")
        print_master("The WDM approach is validated - wavelengths don't interfere.")
        print_master("6 triplets can operate in parallel through the same physical structure.")
    else:
        print_master("WARNING: Some wavelengths showed unexpected behavior.")
        print_master("Check the detailed results for investigation.")
    print_master("=" * 70)
    print_master(f"Completed: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print_master("=" * 70 + "\n")
