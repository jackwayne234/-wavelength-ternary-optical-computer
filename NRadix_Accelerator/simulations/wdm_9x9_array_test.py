#!/usr/bin/env python3
"""
WDM 9×9 Systolic Array Validation Test
=======================================

Tests whether all 6 wavelength triplets can operate through a
9×9 PE array - proving WDM works at the systolic array level.

This is a scaled-up version of the 3×3 test, designed to validate
WDM at larger array sizes before proceeding to 27×27.

The simulation:
1. Creates a 9×9 grid of Processing Elements (simplified)
...
Performance Note:
    ...
    /home/jackwayne/miniconda/envs/meep_env/bin/python wdm_9x9_array_test.py
...
def create_9x9_array():
    """Create the full 9×9 PE array geometry."""
    geometry = []
...
def run_wdm_array_simulation(wavelengths, verbose=True):
    """Run WDM simulation through 9×9 array."""

    print_master(f"\n{'='*70}")
    print_master("WDM 9×9 SYSTOLIC ARRAY SIMULATION")
    print_master(f"{'='*70}")
    ...
    # Create geometry
    geometry, array_span = create_9x9_array()
...
def analyze_array_results(wavelengths, flux_freqs, flux_data_list, output_dir):
    ...
    print_master(f"\n{'='*70}")
    print_master("WDM 9×9 ARRAY SIMULATION RESULTS")
    print_master(f"{'='*70}")
    ...
    if all_passed:
        ...
        print_master("WDM works through the 9×9 systolic array structure.")
    else:
    ...
    if RANK == 0:
        ...
        plt.suptitle('WDM 9×9 Array - Transmission Spectrum at Each Output')
        plt.tight_layout()

        plot_path = os.path.join(output_dir, 'wdm_9x9_array_results.png')
        plt.savefig(plot_path, dpi=150)
        print_master(f"\nPlot saved: {plot_path}")
        plt.close()

        # Save data
        results_path = os.path.join(output_dir, 'wdm_9x9_array_results.txt')
        with open(results_path, 'w') as f:
            f.write("WDM 9×9 SYSTOLIC ARRAY VALIDATION RESULTS\n")
            f.write("=" * 50 + "\n")
...
def main():
    """Run the WDM 9×9 array validation."""

    print_master("\n" + "=" * 70)
    print_master("N-RADIX WDM 9×9 ARRAY VALIDATION TEST")
    print_master("=" * 70)
