#!/usr/bin/env python3
"""
WDM 81x81 Systolic Array Validation Test - FULL CHIP VALIDATION
=================================================================

This is the FINAL PIECE: Full-scale validation proving all 6 wavelength
triplets can operate through a complete 81x81 PE array.

If this works, we have validated WDM at full chip scale. The architecture
is proven from first principles through final implementation.

The simulation:
1. Creates an 81x81 grid of Processing Elements (simplified)
...
Note: For 81 output ports, we use summary visualizations:
- Heatmap showing power received at each port (9x9 grid)
- Aggregate statistics rather than 81 individual plots
...
# Array parameters
ARRAY_SIZE = 81       # 81x81 PE grid - FULL CHIP
...
# Simulation parameters
RESOLUTION = 20       # pixels/um (good balance of accuracy vs runtime)
PML_THICKNESS = 1.5   # um
PADDING = 2.0         # um

# Time - increased for larger array propagation
# Time - must be long enough for light to traverse array
# 81×81 array span = 800 µm, light in LiNbO3 (n=2.2) travels at c/2.2
# Need at least 800 * 2.2 ≈ 1760 time units, use 2000 for margin
RUN_TIME = 2000       # Meep time units
...
def create_81x81_array():
    """Create the full 81x81 PE array geometry."""
    geometry = []
...
def run_wdm_array_simulation(wavelengths, verbose=True):
    """Run WDM simulation through 81x81 array."""

    print_master(f"\n{'='*70}")
    print_master("WDM 81x81 SYSTOLIC ARRAY SIMULATION - FULL CHIP VALIDATION")
    print_master(f"{'='*70}")
    ...
    # Create geometry
    geometry, array_span = create_81x81_array()
...
def analyze_array_results(wavelengths, flux_freqs, flux_data_list, output_dir):
    """Analyze and report results from 81x81 array simulation."""

    measured_wavelengths = [1.0 / f for f in flux_freqs]

    print_master(f"\n{'='*70}")
    print_master("WDM 81x81 ARRAY SIMULATION RESULTS - FULL CHIP VALIDATION")
    print_master(f"{'='*70}")
    ...
    # Summary statistics
    print_master(f"\nOutput Port Statistics (81 ports, 18 wavelengths each):")
    print_master("-" * 60)

    passed_ports = sum(1 for s in port_stats if s['all_positive'])
    failed_ports = 81 - passed_ports

    print_master(f"Ports with all wavelengths detected: {passed_ports}/81")
    print_master(f"Ports with missing wavelengths: {failed_ports}/81")
    ...
    print_master(f"\n{'='*70}")
    if all_passed:
        print_master("SUCCESS: ALL 18 WAVELENGTHS DETECTED AT ALL 81 OUTPUT PORTS!")
        print_master("FULL CHIP WDM VALIDATED - The architecture works at scale.")
        print_master("This is the final piece of the validation puzzle.")
    else:
        print_master("WARNING: Some channels showed low/no power at some ports.")
        print_master(f"  {passed_ports}/81 ports fully operational")
    print_master(f"{'='*70}")

    # Generate visualizations (rank 0 only)
    if RANK == 0:
        # Figure 1: Power heatmap (9x9 grid of 81 ports)
        fig1, axes1 = plt.subplots(1, 2, figsize=(16, 7))

        # Reshape mean powers into 9x9 grid
        power_grid = np.array([s['mean_power'] for s in port_stats]).reshape(9, 9)

        im1 = axes1[0].imshow(power_grid, cmap='viridis', aspect='equal')
        axes1[0].set_title('Mean Power at Each Output Port\n(81 ports as 9x9 grid)', fontsize=12)
        axes1[0].set_xlabel('Port Column (0-8)')
        axes1[0].set_ylabel('Port Row (0-8)')
        ...
        for i in range(9):
            for j in range(9):
                port_num = i * 9 + j + 1
                axes1[0].text(j, i, str(port_num), ha='center', va='center',
                             color='white' if power_grid[i, j] < np.mean(power_grid) else 'black',
                             fontsize=7)

        # Histogram of port powers
        axes1[1].hist(mean_powers, bins=20, color='steelblue', edgecolor='black', alpha=0.7)
        ...
        axes1[1].set_title('Distribution of Output Power Across 81 Ports')
        axes1[1].legend()
        axes1[1].grid(True, alpha=0.3)

        plt.suptitle('WDM 81x81 Full Chip Validation - Output Power Summary', fontsize=14, fontweight='bold')
        plt.tight_layout()

        plot1_path = os.path.join(output_dir, 'wdm_81x81_power_heatmap.png')
        ...
        axes2[0, 0].set_title('Average Transmission Spectrum (all 81 ports)')
        axes2[0, 0].grid(True, alpha=0.3)

        # Spectrum at corner ports (1, 9, 73, 81)
        corner_ports = [0, 8, 72, 80]  # indices
        corner_names = ['Port 1 (corner)', 'Port 9 (corner)', 'Port 73 (corner)', 'Port 81 (corner)']
        ...
        center_port = 40  # Port 41 (center of 9x9)
        ...
        axes2[1, 0].set_title('Center Port (41) Spectrum')
        axes2[1, 0].grid(True, alpha=0.3)

        # Pass/fail grid
        pass_grid = np.array([1 if s['all_positive'] else 0 for s in port_stats]).reshape(9, 9)
        im4 = axes2[1, 1].imshow(pass_grid, cmap='RdYlGn', aspect='equal', vmin=0, vmax=1)
        axes2[1, 1].set_title('Pass/Fail Map\n(Green=All wavelengths detected, Red=Missing)')
        axes2[1, 1].set_xlabel('Port Column (0-8)')
        axes2[1, 1].set_ylabel('Port Row (0-8)')

        for i in range(9):
            for j in range(9):
                port_num = i * 9 + j + 1
                axes2[1, 1].text(j, i, str(port_num), ha='center', va='center',
                                color='white', fontsize=7, fontweight='bold')

        plt.suptitle('WDM 81x81 Full Chip Validation - Spectral Analysis', fontsize=14, fontweight='bold')
        plt.tight_layout()

        plot2_path = os.path.join(output_dir, 'wdm_81x81_spectral_summary.png')
        plt.savefig(plot2_path, dpi=150)
        print_master(f"Spectral summary saved: {plot2_path}")
        plt.close()

        # Save detailed results
        results_path = os.path.join(output_dir, 'wdm_81x81_array_results.txt')
        with open(results_path, 'w') as f:
            f.write("=" * 70 + "\n")
            f.write("WDM 81x81 SYSTOLIC ARRAY VALIDATION RESULTS - FULL CHIP\n")
            f.write("=" * 70 + "\n")
            ...
            f.write(f"\nStatus: {'PASSED - FULL CHIP VALIDATED' if all_passed else 'NEEDS REVIEW'}\n")
            f.write(f"Ports passed: {passed_ports}/81\n")
            f.write(f"\nPower Statistics:\n")
...
def main():
    """Run the WDM 81x81 array validation - FULL CHIP."""

    print_master("\n" + "=" * 70)
    print_master("N-RADIX WDM 81x81 ARRAY VALIDATION TEST - FULL CHIP")
    print_master("=" * 70)
