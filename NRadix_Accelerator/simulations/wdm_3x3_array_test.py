#!/usr/bin/env python3
"""
WDM 3×3 Systolic Array Validation Test
=======================================

Tests whether all 6 wavelength triplets can operate through a small
3×3 PE array - proving WDM works at the systolic array level.

This is a scaled-down version of the full 27×27 test, designed to run
overnight on a local machine with 12 cores.

The simulation:
1. Creates a 3×3 grid of Processing Elements (simplified)
...
Performance Note:
    ...
    /home/jackwayne/miniconda/envs/meep_env/bin/python wdm_3x3_array_test.py
...
# Simulation parameters
RESOLUTION = 30       # pixels/um (lower for faster sim)
PML_THICKNESS = 1.5   # um
PADDING = 2.0         # um

# Time
RUN_TIME = 150        # Meep time units
...
def create_3x3_array():
    """Create the full 3×3 PE array geometry."""
    geometry = []
...
def run_wdm_array_simulation(wavelengths, verbose=True):
    """Run WDM simulation through 3×3 array."""

    print_master(f"\n{'='*70}")
    print_master("WDM 3×3 SYSTOLIC ARRAY SIMULATION")
    print_master(f"{'='*70}")
    ...
    # Create geometry
    geometry, array_span = create_3x3_array()
...
def analyze_array_results(wavelengths, flux_freqs, flux_data_list, output_dir):
    """Analyze and report results from array simulation."""

    measured_wavelengths = [1.0 / f for f in flux_freqs]

    print_master(f"\n{'='*70}")
    print_master("WDM 3×3 ARRAY SIMULATION RESULTS")
    print_master(f"{'='*70}")
    ...
    if all_passed:
        ...
        print_master("WDM works through the 3×3 systolic array structure.")
    else:
    ...
    if RANK == 0:
        ...
        for port_idx, (ax, flux_data) in enumerate(zip(axes, flux_data_list)):
        ...
        plt.suptitle('WDM 3×3 Array - Transmission Spectrum at Each Output')
        plt.tight_layout()

        plot_path = os.path.join(output_dir, 'wdm_3x3_array_results.png')
        plt.savefig(plot_path, dpi=150)
        print_master(f"\nPlot saved: {plot_path}")
        plt.close()

        # Save data
        results_path = os.path.join(output_dir, 'wdm_3x3_array_results.txt')
        with open(results_path, 'w') as f:
            f.write("WDM 3×3 SYSTOLIC ARRAY VALIDATION RESULTS\n")
            f.write("=" * 50 + "\n")
            f.write(f"Date: {datetime.now()}\n")
            f.write(f"Array: {ARRAY_SIZE}×{ARRAY_SIZE}\n")
...
def main():
    """Run the WDM 3×3 array validation."""

    print_master("\n" + "=" * 70)
    print_master("N-RADIX WDM 3×3 ARRAY VALIDATION TEST")
    print_master("=" * 70)
